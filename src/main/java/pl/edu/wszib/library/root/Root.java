...
import static pl.edu.wszib.library.gui.GUI.showLoginRegister;

public class Root {
    private static final Scanner scanner = new Scanner(System.in);
    final BookDAO bookDAO = BookDAO.getInstance();
    final pl.edu.wszib.library.root.Auth auth = pl.edu.wszib.library.root.Auth.getInstance();
        ...
        while(isWorking) {
            switch(showLoginRegister()){
                case "1" -> {
                    ...
                    if(!pl.edu.wszib.library.root.Auth.validate(user)){
                        System.out.println("Wrong login or password entered. Valid login and password consist of at least 5 letters,");
                    }else{
                        if(pl.edu.wszib.library.root.Auth.doesExist(user, this.auth.userDAO)) {
